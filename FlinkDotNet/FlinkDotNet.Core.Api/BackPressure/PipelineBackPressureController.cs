...
/// <summary>
/// FlinkDotnet 2.0 style pipeline back pressure controller that manages back pressure
...
public class PipelineBackPressureController : IDisposable
{
        ...
        PipelineBackPressureConfiguration? config = null)
    {
        ...
        // Start monitoring back pressure every 2 seconds (FlinkDotnet 2.0 style)
        _monitoringTimer = new Timer(MonitorPipelineBackPressure, null, 
            TimeSpan.FromSeconds(2), TimeSpan.FromSeconds(2));
        
        _logger.LogInformation("PipelineBackPressureController initialized for FlinkDotnet 2.0 style pipeline back pressure");
    }
    ...
    /// <summary>
    /// Get stage-specific configuration based on FlinkDotnet 2.0 best practices
    /// </summary>
    ...
    /// <summary>
    /// Monitor pipeline back pressure and apply FlinkDotnet 2.0 style throttling
    /// </summary>
    ...
    /// <summary>
    /// Calculate overall pipeline pressure using FlinkDotnet 2.0 heuristics
    /// </summary>
    ...
    private void ApplyPipelineBackPressureDecisions(double overallPressure)
    {
        foreach (var (stageName, flowControl) in _stageFlowControls)
        {
            if (_stageMetrics.TryGetValue(stageName, out var metrics))
            {
                ...
                // Apply FlinkDotnet 2.0 style throttling decisions
...
/// <summary>
/// Pipeline stage names for FlinkDotnet 2.0 style processing
/// </summary>
